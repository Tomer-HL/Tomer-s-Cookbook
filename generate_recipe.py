...
RECIPE_NAME   = "Arayes"         # שם קובץ (ללא סיומת)
CATEGORY      = "entrees"          # entrees | sides | bread | desserts
TIME_EN       = "50 minutes"
TIME_HE       = "50 דקות"
...
/* תווית מובילה inline בתוך שורה (כמו "Tip:" ב-"Tip: Freeze portions...") */
.inline-label {{
    color: var(--orange);
    font-weight: 700;
}}"""
...
# מזהה תבנית "Label: content" בתחילת שורה: תווית קצרה (עד 40 תווים) שאחריה
# נקודותיים, רווח, ותוכן באותה השורה. נועד להדגיש ביטויים מובילים כמו
# "Tip: ...", "Yield: ...", "טיפ: ..." בלי לשבור אותם לשורה נפרדת.
# מחריג '*' ו-'<' כדי לא להתנגש עם **bold** או עם תגיות HTML שכבר נכנסו.
_INLINE_LABEL_RE = re.compile(r"(?m)^(?P<label>[^\n:*<]{1,40}):[ \t]+(?=\S)")
...
def _bold_inline_labels(text: str) -> str:
    """מדגיש 'Label:' בתחילת שורה גם כשיש תוכן באותה שורה (למשל "Tip: ...").
    יש להפעיל את הפונקציה הזו *לפני* _format_inline כדי לא להתנגש עם תגיות
    <strong> שמופקות מ-**bold** של מרקדאון."""
    return _INLINE_LABEL_RE.sub(
        lambda m: f'<strong class="inline-label">{m.group("label")}:</strong> ',
        text,
    )
...
def _render_ingredients(ingredients: list[str]) -> str:
    ...
    בנוסף, ** ** מומר ל-<strong>, וגם "Label: content" בתחילת שורה מקבל הדגשה inline."""
    out: list[str] = []
    for item in ingredients:
        stripped = item.strip()
        is_heading = stripped.endswith(":") and len(stripped) <= 80
        if is_heading:
            formatted = _format_inline(stripped.rstrip(":"))
            out.append(f'<li class="ing-heading">{formatted}</li>')
        else:
            # קודם הדגשת תוויות inline (לפני _format_inline כדי לא להתנגש עם **bold**)
            with_labels = _bold_inline_labels(stripped)
            formatted   = _format_inline(with_labels)
...
def _render_steps(steps: list[str]) -> str:
    ...
    בנוסף, ** ** מומר ל-<strong>, וגם "Label: content" בתחילת שורה (כמו "Tip: ...",
    "Yield: ...") מקבל הדגשה inline בלי לשבור לשורה נפרדת."""
    out: list[str] = []
    for step in steps:
        heading, body = _split_subheading(step)
        # קודם הדגשת תוויות inline בגוף הצעד (לפני _format_inline)
        body      = _bold_inline_labels(body)
